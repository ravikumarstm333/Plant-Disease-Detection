...
def get_chatbot_response(question, disease_name=None):
    ...
    if disease_data:
        ...
        if question == "" or 'analyze' in question or 'photo' in question or 'image' in question:
            return (
                f"I detected {disease_display_name} in your uploaded photo. "
                f"Recommended treatment: {treatment} "
                f"Suitable fertilizer: {fertilizer}. "
                "Ask me if you want more details about symptoms, prevention, or crop care."
            )
